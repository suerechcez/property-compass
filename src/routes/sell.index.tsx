...
function CardIcon({ png, jpg, alt, fallback: Fallback }: { png: string; jpg: string; alt: string; fallback: typeof Handshake }) {
  const [src, setSrc] = useState(png);
  const [failed, setFailed] = useState(false);
  if (failed) return <div className="grid h-14 w-14 place-items-center rounded-full bg-primary/10 text-primary"><Fallback className="h-7 w-7" /></div>;
  return <div className="grid h-24 w-24 place-items-center"><img src={src} alt={alt} className="h-full w-full object-contain" onError={() => { if (src === png) setSrc(jpg); else setFailed(true); }} /></div>;
}
